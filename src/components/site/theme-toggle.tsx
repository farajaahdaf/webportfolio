...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        "relative inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background/90 shadow-sm backdrop-blur-sm transition-[transform,background-color,border-color,box-shadow] duration-[160ms] ease-[cubic-bezier(0.23,1,0.32,1)] active:scale-[0.97] hover:border-primary/40 hover:bg-secondary cursor-pointer dark:bg-background/40",
        className
      )}
    >
      <Sun
        className={cn(
          "absolute h-4 w-4 transition-[transform,opacity] duration-[180ms] ease-[cubic-bezier(0.23,1,0.32,1)]",
